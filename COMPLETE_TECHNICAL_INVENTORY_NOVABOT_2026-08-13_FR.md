...
## Actualisation technique consolidée au 14 août 2026

Cette passe a été recalculée sur le code actif de la racine, en excluant les sauvegardes, les artefacts Python générés et les rapports de travail du dossier `Markdown`. Elle fait autorité sur les nombres et branchements du socle du 13 août conservé plus bas.

### Comptage AST actuel

| Élément | Applicatif/technique | Tests | Total |
|---|---:|---:|---:|
| Fichiers Python | 171 | 77 | 248 |
| Classes | 214 | 195 | 409 |
| Fonctions hors classes | 399 | 37 | 436 |
| Méthodes et callbacks dans une portée de classe | 2 142 | 1 482 | 3 624 |
| Constructeurs | 117 | 66 | 183 |
| Définitions asynchrones | 46 | 21 | 67 |
| Propriétés | 37 | 0 | 37 |
| Dataclasses | 44 | 0 | 44 |
| Enums | 6 | 0 | 6 |
| Slots PyQt décorés | 11 | 0 | 11 |
| Signaux PyQt déclarés | 36 | 0 | 36 |
| Sites `QTimer`/`singleShot` | 29 | 0 | 29 |
| Constructeurs explicites Thread/Process/Popen | 8 | 0 | 8 |

Les méthodes incluent les callbacks locaux déclarés lexicalement sous une classe. Les appels dynamiques, connexions Qt et handlers enregistrés ne sont pas assimilés à des appels absents. Le comptage des threads/processus de cette passe porte uniquement sur les constructeurs explicites et n’est donc pas directement comparable à une recherche textuelle plus large.

### Nouveaux composants et responsabilités vérifiés

| Fichier | Définitions ou responsabilité | Relations principales | État |
|---|---|---|---|
| `app/core/module_scroll_viewport.py` | `ModuleScrollViewport` | enveloppe Telegram, MetaTrader, Dashboard et Copy Trader depuis `main.py` | UI active |
| `app/core/display_policy.py` | `HistoryPanelPlan`, `build_history_panel_plan` | calcule la hauteur non prioritaire de la console globale | politique UI |
| `app/core/display_adaptation.py` | adaptation des fenêtres et dialogues | applique mode d’affichage et typographie aux dialogues applicatifs | infrastructure UI |
| `app/core/i18n.py` | chargement du domaine technique | lit `language/*/technical.json` | infrastructure active |
| `app/mt4/mt4_connection_worker.py` | connexion MT4 annulable hors UI | construit/ouvre `MT4PlatformSession` | worker Qt actif |
| `app/mt4/mt4_platform_session.py` | session MT4 et répertoire d’authentification | worker MT4, transport, adaptateur, erreurs de connexion | actif |
| `app/workers/mt4_worker.py` | INI profil, endpoints dynamiques, ownership et identité READY | terminal MT4, ZeroMQ, EA | processus actif |
| `app/copy_trader/mt4_shared_source.py` | snapshots/diffs MT4 sans cache persistant | session MT4 principale, Copy Trader MT4→MT5 | actif facultatif |
| `app/trading/execution_admission_policy.py` | règle `should_adjust_take_profits_to_actual_entry` | exécutions et lifecycles MT4/MT5 | policy active |

### Ports et propriété du bridge MT4

`app/workers/mt4_worker.py` conserve 6001/6002 comme première paire compatible, puis parcourt jusqu’à 32 paires. Chaque paire progresse de deux ports. Le worker crée un fichier d’ownership par port d’événement dans le répertoire temporaire Windows, publie les endpoints actifs et ne récupère automatiquement qu’un worker dont la propriété est suffisamment prouvée. L’EA reçoit les endpoints choisis ; les constantes historiques restent des valeurs par défaut, pas une exclusivité globale.

### Authentification et persistance MT4

`MT4ConnectionWorker` reçoit `authentication_dir`; `MT4PlatformSession` le transmet au worker de processus ; `MT4TerminalProcessManager` y crée un INI temporaire `NovaBOT_MT4_Login_*.ini`, nettoie les fichiers orphelins et supprime le fichier courant après usage. Pour le module principal comme pour le Copy Trader, ce répertoire est `SECRETS_DIR/mt4_runtime`, donc propre au profil actif.

Les identifiants persistants restent la responsabilité de `MT4CredentialsStore`. Le fichier INI n’est pas une nouvelle base de compte : c’est un artefact transitoire du lancement officiel du terminal.

### Observation et performance MT4

`MT4PlatformAdapter` expose un snapshot et des faits normalisés. `MT4TradeMonitoringMixin` réutilise les lectures cohérentes pendant un cycle ; `MT4SharedSourceObserver` diffère des snapshots live sans fichier de cache ; le Copy Trader borne le nombre d’événements traités par passage UI. Le worker utilise un journal rotatif et limite les répétitions de messages identiques.

### Lifecycle MT4 renforcé

`MT4TradeLifecycleMixin` gère désormais explicitement la promotion pending→position dans le batch existant, les recherches d’historique par ticket, les preuves partielles et les réévaluations différées. `MT4TradeClassifierMixin` utilise les niveaux initiaux, le prix de sortie, le type d’action et l’historique disponible pour distinguer TP, SL, BE, sécurisé, Smart et manuel. L’EA expose les requêtes live nécessaires ; l’historique chargé dans MT4 demeure une limite externe observable.

### UI et console globale

`main.py` conserve les références des modules réels tout en ajoutant leur `ModuleScrollViewport` aux onglets. `_refresh_visible_module` délègue au widget contenu. Le plan de console global est recalculé lors des changements de taille/mode et les constantes actuelles fixent deux lignes visibles dans tous les modes. La console garde sa barre de défilement et ne force plus une hauteur au détriment du module.

### Langues

Dix fichiers `technical.json` ont été ajoutés, un par langue. Les clés techniques sont vérifiées par `test_connection_technical_i18n_contract.py` et les contrats de notifications restent vérifiés séparément. Les descriptions de l’ajustement des TP sont présentes dans `documentation.json` et les résumés visibles dans `mt5.json` pour les dix langues.

### Tests ajoutés depuis le socle du 13 août

Les fichiers de tests applicables comprennent notamment :

- `test_module_scroll_viewport.py` ;
- `test_connection_technical_i18n_contract.py` ;
- `test_mt4_graceful_shutdown.py` et les extensions de `test_mt4_worker_characterization.py` ;
- les extensions de `test_mt4_primary_platform_architecture.py` ;
- les extensions de `test_copy_trader_mt4_shared_source.py` et `test_copy_trader_poll_characterization.py` ;
- les extensions de `test_display_adaptation.py` ;
- les extensions de `test_mt5_order_execution_characterization.py` et `test_mt5_trade_lifecycle_characterization.py` couvrant aussi le parcours MT4.

État complet le plus récent documenté : 1 016 tests, 30 échecs d’assertion historiques, 0 erreur.

## Socle technique consolidé au 13 août 2026 (historique conservé)

Cette section restitue le parcours AST et les branchements vérifiés au 13 août 2026. Elle est conservée pour la traçabilité ; les nombres et compléments du 14 août placés en tête du document font autorité.

### Comptage actuel

| Élément | Applicatif/technique | Tests | Total |
|---|---:|---:|---:|
| Fichiers Python | 170 | 75 | 245 |
| Classes | 211 | 191 | 402 |
| Fonctions hors classes | 387 | 37 | 424 |
| Méthodes et callbacks dans une portée de classe | 2 111 | 1 431 | 3 542 |
| Constructeurs | 115 | 65 | 180 |
| Async | 46 | 21 | 67 |
| Propriétés | 36 | 0 | 36 |
| Dataclasses | 43 | 0 | 43 |
| Enums | 6 | 0 | 6 |
| Slots décorés | 11 | 0 | 11 |
| Signaux PyQt déclarés | 36 | 0 | 36 |
| Sites QTimer/singleShot | 25 | 0 | 25 |
| Sites thread/processus détectés | 12 | 2 | 14 |

Le comptage des méthodes utilise la portée de classe AST et inclut les callbacks locaux déclarés dans une méthode. Il ne prétend pas mesurer les appels dynamiques.

### Nouveaux packages structurants

| Package | Propriétaire technique | Dépendances majeures | État |
|---|---|---|---|
| `app.trading` | contrats et services communs MT4/MT5 | dataclasses, Protocol, stores profil | infrastructure active |
| `app.mt4` | moteur MT4 principal | worker, ZeroMQ, EA, PyQt pour connexion | actif |
| `app.copy_trader.mt4_shared_source` | observation MT4 principale vers cible MT5 | session MT4 live, mapping Copy Trader | actif facultatif |
| `app.metatrader` | namespace de convergence | aucun symbole métier propre | infrastructure |

### Inventaire du moteur MT4 principal

| Fichier | Définitions structurantes | Rôle réel |
|---|---|---|
| `mt4_composition.py` | `MT4DomainMixins` | compose exécution, actions, Smart, math, monitoring, classifier, messages et lifecycle MT4 |
| `mt4_trading_engine.py` | `MT4TradingEngine` | façade de domaine attachée au widget MetaTrader |
| `mt4_platform_session.py` | `MT4PlatformSession` | possède transport, adaptateur et cycle connexion/déconnexion |
| `mt4_platform_adapter.py` | `MT4QueueTransport`, `MT4PlatformAdapter` | transforme requêtes/réponses worker en modèles de trading ; timeouts et erreurs dédiés |
| `mt4_connection_worker.py` | `MT4ConnectionWorker(QThread)` | ouvre la session hors du thread UI et émet résultat/erreur |
| `mt4_credentials_store.py` | `MT4CredentialsStore` | chiffre et restaure les identifiants MT4 propres au profil |
| `mt4_order_transport.py` | `MT4OrderTransport` | OPEN et modifications pending via adaptateur |
| `mt4_position_transport.py` | `MT4PositionTransport` | clôture et modification des positions |
| `mt4_order_execution.py` | `MT4OrderExecutionMixin` | admission finale, volumes, MARKET/LIMIT/STOP, batch composite et notifications d’exécution |
| `mt4_position_actions.py` | `MT4PositionActionsMixin` | BE, modifications, clôtures et suppression pending |
| `mt4_smart_close.py` | `MT4SmartCloseMixin` | clôture Smart totale/partielle et agrégation des résultats |
| `mt4_trade_math.py` | `MT4TradeMathMixin` | conversions numériques, volume et tolérances propres aux faits MT4 |
| `mt4_trade_monitoring.py` | `MT4TradeMonitoringMixin` | observation périodique des positions, pending et ticks |
| `mt4_trade_classifier.py` | `MT4TradeClassifierMixin` | preuve et cause des sorties MT4 |
| `mt4_trade_messages.py` | `MT4TradeMessagesMixin` + façades `build_*` | messages de lifecycle, Smart, pending et fin |
| `mt4_trade_lifecycle.py` | `MT4TradeLifecycleMixin` | batchs, corrélations, BE, TP, pending, fin et publications |
| `mt4_state_reconciler.py` | `MT4StateReconciler` | ordonne/réconcilie les snapshots live |
| `mt4_bridge_installer.py` | résultats/status/compilateur/processus | bundle, résolution data folder, vérification, compilation et lancement installateur |
| `mt4_post_install_configurator.py` | `MT4PostInstallConfigurator` | configuration Windows assistée après installation |
| `MQL4/Experts/NovaBot_MT4_Slave_ZMQ.mq4` | EA MT4 | compte, terminal, symboles, specs, ticks, rates, positions, pending, historique et actions |

### Contrats du bridge MT4

Le worker `app/workers/mt4_worker.py` possède le lancement exact du terminal, le fichier INI temporaire de login, le propriétaire du port, la récupération prudente d’un worker invisible, les sockets ZeroMQ et la traduction queues/JSON.

L’EA expose notamment :

- `GET_ACCOUNT`, `GET_TERMINAL`, `GET_SYMBOLS`, `SELECT_SYMBOL`, `GET_SYMBOL_SPEC`, `GET_TICK`, `GET_RATES`, `GET_POSITIONS`, `GET_PENDING`, `GET_HISTORY` ;
- `OPEN`, `CLOSE_TICKET`, `CLOSE_VOLUME`, `DELETE_PENDING`, `MODIFY_SLTP`, `MODIFY_PENDING` ;
- `READY`, `HEARTBEAT`, snapshots, réponses corrélées, erreurs et événements d’action.

`MT4QueueTransport` associe chaque requête à un identifiant. `MT4PlatformAdapter` convertit les réponses en `PlatformAccount`, `TerminalStatus`, `SymbolSpec`, `Quote`, `Candle`, `OpenPosition`, `PendingOrder`, `HistoricalExecution` et `ExecutionReceipt`.

### Couche commune `app.trading`

| Fichier | Contenu |
|---|---|
| `models.py` | enums plateforme/sens/type et dataclasses de compte, terminal, marché, opérations, commandes, receipts et événements |
| `ports.py` | Protocols session, faits broker, marché, actions et événements |
| `platform_registry.py` | enregistrement des services MT4/MT5 et choix actif |
| `runtime_state.py` | état de connexion, compte, terminal, génération et snapshot thread-safe |
| `platform_profile_store.py` | persistance de la plateforme active par profil |
| `platform_text.py` | adaptation des textes MT4/MT5 |
| `money_management_volume_planner.py` | propriétaire commun du calcul de volumes |
| `trade_numeric.py`, `trade_protection_policy.py`, `progressive_tp.py` | comportements réellement mutualisés |
| `lifecycle_*.py` | état runtime, événement terminal, transition, publication, acknowledgement et watchlist |
| `dashboard_terminal_*.py` | preuves et archivage passifs pour le Dashboard |
| `metatrader_protocol.py`, `trade_protocol.py` | contrats/façades de compatibilité |

Les modules homonymes sous `app.mt5` qui ne définissent plus de symbole sont des façades de compatibilité réexportant la couche commune.

### Copy Trader MT4 partagé

`mt4_shared_source.py` définit `SharedSourceEvent`, normalise position/pending, compare deux snapshots et observe la session MT4 principale. Le tracker traite :

- nouvelle position ou pending ;
- mise à jour ;
- disparition/clôture ;
- transformation pending→position ;
- remplacement de ticket après clôture partielle.

Le remplacement partiel transfère le mapping de l’ancien ticket au résiduel et envoie une réduction de volume à la cible MT5. Il ne doit pas produire `OPEN`. La matrice interdit MT4→MT4 et conserve MT5→MT5/MT4.

### Propriété des états

| État | Propriétaire | Source de vérité |
|---|---|---|
| plateforme active | `MT5App` + `TradingPlatformRegistry` | session connectée et profil |
| connexion MT4 | `MT4PlatformSession`/`MT4ConnectionWorker` | `READY`, heartbeat et identité EA |
| faits broker MT4 | `MT4PlatformAdapter` | réponses live EA |
| batchs MT4 | `MT4TradeLifecycleMixin` | inventaires/événements live + état runtime |
| reprise MT4 | stores dédiés | aide de reprise, jamais vérité terminale |
| mapping Copy Trader | `CopyTraderMT5` | association persistante validée contre les inventaires |
| analytics | `DashboardAnalyticsStore` | projection SQLite, jamais décision d’exécution |

### Persistance MT4 et temporaire

- `data/trading_platform.json` : plateforme choisie ;
- identifiants MT4 chiffrés et clé sous le profil ;
- `data/be_watchlist_mt4.json` : reprise BE MT4 séparée ;
- journaux console et worker propres au profil ;
- `%TEMP%/NovaBot_MT4_bridge_6002.json` : propriétaire technique du bridge ;
- fichier INI de connexion MT4 temporaire et supprimé après usage ;
- bundle temporaire de l’installateur ;
- archive `app/mt4/vendor/mql-zmq-802c86c9e3647c917151ff724ffd15baa9bc152c.zip` embarquée pour éviter une dépendance réseau obligatoire.

Aucun de ces fichiers ne remplace les faits live de la plateforme.

### Tests ajoutés ou étendus

La suite contient désormais 75 fichiers. Les domaines MT4 ajoutés sont notamment `test_mt4_primary_platform_architecture`, `test_mt4_connection_runtime_configuration`, `test_mt4_graceful_shutdown`, `test_metatrader_engine_separation`, `test_mt4_bridge_installer`, `test_mt4_worker_characterization` et `test_copy_trader_mt4_shared_source`.

Résultat observé avec l’environnement Python du projet : 978 tests, 26 échecs d’assertion, 0 erreur. La compilation réussit. Les échecs sont documentés comme état existant et ne sont ni corrigés ni filtrés ici.


Date de l'inventaire : 13 août 2026  
Source de vérité : code présent dans le dossier analysé  
Nature : cartographie statique et dynamique de référence, sans modification du code

## 1. Périmètre et méthode

L'inventaire couvre l'intégralité des 364 fichiers utiles de l'arborescence fournie, en excluant seulement les artefacts Python générés (`__pycache__`, `.pyc`). Les documents existants ont servi de contrôle croisé ; les nombres, symboles, imports, écritures, branchements et contrats indiqués ci-dessous ont été vérifiés dans le code actuel.

La méthode combine :

- parcours de l'arborescence et classification par extension ;
- analyse AST des 245 fichiers Python : définitions, signatures, décorateurs, héritage, imports, constantes, signaux, fonctions asynchrones et sites de création de timers/processus ;
...
- lecture des 75 modules de test et de leurs 978 scénarios ;
...
| Fichiers Python | 170 | 75 | 245 |
| Classes | 211 | 191 | 402 |
| Fonctions | 387 | 37 | 424 |
| Méthodes, callbacks locaux inclus | 2 111 | 1 431 | 3 542 |
| Constructeurs `__init__` | 115 | 65 | 180 |
| Fonctions/méthodes `async` | 46 | 21 | 67 |
| Propriétés `@property` | 36 | 0 | 36 |
| Dataclasses | 43 | — | 43 |
| Enums | 6 | — | 6 |
| Slots décorés `@pyqtSlot` | 11 | — | 11 |
| Signaux PyQt déclarés | 36 | — | 36 |

L'arborescence contient aussi 80 JSON de langue, 18 PNG, 9 Markdown, 5 BAT, 2 PowerShell, 1 SPEC PyInstaller, 1 ICO, 1 EA MQ4, 1 notice texte et 1 archive mql-zmq embarquée.
...
| `app.copy_trader` | Orchestrateur MT5→MT5/MT4 et source partagée MT4→MT5. | `main`. | actif/facultatif |
| `app.workers` | Processus source/cible, worker MT5 et pont MT4 ZeroMQ. | Copy Trader. | infrastructure |
...
La suite `unittest` contient 75 fichiers et 978 scénarios détectés. Le tableau précise le domaine, les composants principalement traversés et les comportements caractérisés. Un test « AST » inspecte la structure/source sans nécessairement instancier l'application.
...
État relevé sur cette même arborescence pendant l'analyse : **978 tests exécutés, 26 échecs d'assertion, 0 erreur**. Les échecs se concentrent sur des contrats historiques About, classifier/lifecycle/messages/monitoring, onboarding/progressive et surtout Telegram Filter. Ils ne sont ni corrigés ni masqués dans cet inventaire. La compilation `python -m compileall app validation main.py` a réussi.
...
1. **Fichiers analysés : 364** fichiers utiles.
2. **Fichiers Python applicatifs/techniques : 170**.
3. **Fichiers de tests : 75**.
4. **Classes applicatives/techniques : 211** ; 402 avec les tests.
5. **Fonctions applicatives/techniques : 387** ; 424 avec les tests.
6. **Méthodes applicatives/techniques : 2 111** ; 3 542 avec les tests, selon le comptage AST indiqué.
7. **Signaux PyQt : 36** ; **slots décorés : 11**.
8. **Threads/workers/processus : 12 sites de création applicatifs détectés par AST**, auxquels s'ajoutent la boucle asyncio dédiée et les processus externes terminal/installateur.
9. **Timers : 25 sites** : huit constructions QTimer et dix-sept `singleShot`.
...
13. **Propriétaires d'état :** profile manager, TelegramApp/stores, registre/runtime de plateforme, lifecycles MT4 et MT5 séparés, MoneyManagementController, DashboardAnalyticsStore, CopyTraderMT5 et SupervisionRuntime.
14. **Chemins principaux :** Telegram→filtre→parser→alias→admission/MM/validation→transport MT4 ou MT5 sélectionné→lifecycle de plateforme→outbox Telegram→Dashboard/supervision ; Smart reply→resolver→gateway de plateforme ; source MT5→MT5/MT4 ou source MT4 partagée→MT5.
