...
NovaBOT is a Windows PyQt5 desktop application that connects Telegram to MetaTrader 5, tracks trade lifecycles, and can replicate an MT5 source account to MT5 or MT4.
...
The main window has Telegram, MetaTrader 5, Dashboard, and Copy Trader tabs. The identity bar displays profile, listened sources, MT5 state, profile-status icon, and setup/status button. The button is red when no setup step is ready, orange when partially ready, and green when complete. Closing stops Copy Trader workers, timers, Telegram loop, supervision observer/transport, and releases the profile lock.
