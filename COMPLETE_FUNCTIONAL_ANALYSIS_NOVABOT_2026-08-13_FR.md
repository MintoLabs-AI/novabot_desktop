...
## Actualisation consolidée au 14 août 2026

Cette actualisation est fondée sur le code actif de la racine du projet au 14 août 2026. Elle complète et, en cas d’écart, remplace les formulations de l’état consolidé du 13 août conservé plus bas pour traçabilité.

### Périmètre actuel vérifié

- 376 fichiers utiles hors archives de sauvegarde, rapports du dossier `Markdown`, `__pycache__` et `.pyc` ;
- 248 fichiers Python : 171 applicatifs/techniques et 77 tests ;
- 90 JSON de langue, 18 PNG, 9 Markdown de référence à la racine, 5 BAT, 2 PowerShell, 1 SPEC, 1 ICO, 1 MQ4 et 1 notice ;
- dernière suite complète documentée : 1 016 tests, 30 échecs d’assertion historiques, 0 erreur ;
- les évolutions décrites ci-dessous sont caractérisées par des tests ciblés en plus de la suite complète.

### Lifecycle MT4 approfondi et parité MT5

Le lifecycle MT4 reste un moteur distinct de MT5. Il a été renforcé sans faire de MT5 un chemin d’exécution MT4 :

- un pending MT4 déclenché est promu dans le batch logique existant au lieu de créer un second lifecycle ;
- les branches MARKET et LIMIT issues du même signal restent corrélées au même message Telegram et au même batch métier ;
- l’historique est recherché par ticket, puis réconcilié avec les inventaires live ;
- TP, SL initial, Break Even, SL sécurisé, Smart Close, fermeture manuelle, clôture partielle et suppression pending sont classés à partir des preuves MT4 disponibles ;
- une clôture partielle ne devient pas artificiellement une nouvelle position et n’est pas recomptée comme clôture terminale ;
- les notifications intermédiaires propres à MT4 ne remplacent pas les notifications contractuelles de lifecycle ;
- l’option « Autoriser les LIMIT après TP1 » reste la source de vérité pour la conservation ou l’annulation des LIMIT sœurs.

La lecture de l’historique MT4 reste limitée à l’historique chargé par le terminal. Une preuve live temporairement indisponible provoque un report ou une cause explicitement indéterminée, jamais une cause inventée.

### Connexion MT4, identité et allocation du pont

La connexion MT4 repose sur l’EA et ZeroMQ, puis sur `READY` et les heartbeats du compte observé. Les renforcements actuels sont :

- authentification du compte par fichier INI temporaire dans le répertoire secret propre au profil (`secrets/mt4_runtime`) ;
- vérification de l’identité réellement publiée par l’EA avant de déclarer la session connectée ;
- association du bridge avec le terminal réellement lancé et refus explicite d’un terminal/compte différent ;
- connexion exécutée hors du thread graphique et annulable ;
- détection rapide d’une divergence de compte confirmée ;
- allocation automatique d’une paire de ports ZeroMQ libre lorsqu’une autre instance NovaBOT utilise déjà la paire historique 6001/6002 ;
- transmission des endpoints attribués à l’EA, sans fermer ni détourner le bridge d’une autre instance visible.

Pywinauto reste limité à l’installation/configuration du pont. Il n’est pas le mécanisme de connexion du compte MT4.

### Réactivité MT4 et Copy Trader

Les lectures MT4 coûteuses ont été regroupées autour d’un snapshot live réutilisable pendant un cycle court. Le Copy Trader peut consommer l’observateur MT4 partagé au lieu de lancer une seconde observation complète de la même source. Les files UI sont traitées avec un budget borné afin de rendre la main à Qt entre deux lots d’événements. La journalisation répétitive du worker est limitée sans supprimer les erreurs ni transitions importantes.

Ces mécanismes sont des optimisations de transport et d’observation. Le cache n’est toujours pas une source de vérité pour le tick, le compte, les positions, les pending, l’historique ou les causes de clôture.

### Copy Trader et compte du module MetaTrader

L’option « Utiliser le compte du module MetaTrader » est initialisée dès la construction du Copy Trader, y compris lorsque le module actif est MT4. Le parcours MT4 charge l’identité live et la session partagée du module principal ; il ne dépend plus d’un cycle manuel décocher/recocher. La matrice reste inchangée : MT5→MT5, MT5→MT4 et MT4→MT5 sont permis ; MT4→MT4 reste interdit.

### Défilement et adaptation de l’interface

Les quatre modules principaux sont enveloppés par `ModuleScrollViewport`. Le contenu conserve sa taille naturelle ; des barres horizontale ou verticale apparaissent lorsque le mode d’affichage, la taille de police ou la fenêtre ne permettent plus de tout afficher. Le module réel reste la cible des rafraîchissements malgré l’enveloppe de défilement.

La console globale inférieure n’est plus prioritaire sur le contenu des modules. Sa hauteur est calculée par `display_policy` selon l’espace disponible et elle est actuellement limitée à deux lignes visibles ; le reste demeure accessible par défilement. Les dialogues continuent d’utiliser l’adaptation d’affichage commune, hors gestionnaire de profils conformément à son contrat particulier.

### Messages techniques et langues

Les messages de connexion et les erreurs techniques concernés passent par le domaine `technical.json`, présent dans les dix langues. Les différences fonctionnelles légitimes restent visibles : MT4 parle d’AutoTrading, d’EA, de DLL et de bridge ; MT5 parle de trading algorithmique et de l’API MT5. L’harmonisation porte sur la structure et la traduction, pas sur l’effacement de ces différences.

### Ajustement sécurisé des TP à l’entrée réelle

L’option Money Management « Ajuster les TP à l’entrée réelle » conserve la référence prévue et peut être réévaluée par le lifecycle, mais son application est désormais soumise à une règle d’admission unique :

- BUY : ajustement uniquement tant que le prix courant exécutable reste strictement inférieur à l’entrée réelle ;
- SELL : ajustement uniquement tant que le prix courant exécutable reste strictement supérieur à l’entrée réelle ;
- prix égal, déjà favorable ou tick inutilisable : aucun ajustement ;
- BUY utilise le bid et SELL l’ask pour la décision de sécurité ;
- les ordres STOP restent exclus ; le SL n’est pas modifié.

La règle est propriétaire de `ExecutionAdmissionPolicy` et est appliquée de manière cohérente par les exécutions et lifecycles MT4 et MT5. La documentation utilisateur et les résumés UI ont été mis à jour dans les dix langues.

## Socle consolidé au 13 août 2026 (historique conservé)

Cette section restitue l’état vérifié au 13 août 2026. Elle est conservée pour la traçabilité ; l’actualisation du 14 août placée en tête du document fait autorité en cas d’écart.

### Périmètre recalculé

- 364 fichiers utiles hors `__pycache__` et `.pyc` ;
- 245 fichiers Python : 170 applicatifs/techniques et 75 tests ;
- 80 JSON de langue, 18 PNG, 9 Markdown, 5 BAT, 2 PowerShell, 1 SPEC, 1 ICO, 1 MQ4, 1 notice et 1 archive mql-zmq ;
- compilation de `app`, `validation` et `main.py` réussie ;
- 978 tests, 26 échecs d’assertion, 0 erreur.

### Architecture de trading bi-plateforme

L’onglet MetaTrader sélectionne MT5 ou MT4 puis une installation correspondante. `MT5App` reste la façade UI historique, mais `active_platform` sélectionne deux moteurs :

- MT5 : `MT5DomainMixins`, API `MetaTrader5`, `MT5BrokerFacts`, transports et lifecycle MT5 ;
- MT4 : `MT4TradingEngine`, `MT4DomainMixins`, `MT4PlatformSession`, `MT4PlatformAdapter`, transports et lifecycle MT4 ;
- `app.trading` : modèles, ports et services mutualisables ;
- `TradingPlatformRegistry` et `TradingRuntimeState` : session et état associés à la plateforme.

MT4 ne détourne pas l’API MT5 ; MT5 ne traverse pas ZeroMQ.

### Parcours MT4 principal

`Telegram → filtre → parser → alias → admission/MM → validation → MT4OrderExecutionMixin → MT4OrderTransport → MT4PlatformAdapter → MT4QueueTransport → worker ZeroMQ → EA MT4 → terminal → inventaires/événements → lifecycle MT4 → outbox → Dashboard/supervision`.

Le parser, le MM et l’admission restent applicatifs. Les faits broker et actions terminales sont spécifiques.

### Connexion MT4 et pont

La connexion exige installation réelle, EA `.mq4`/`.ex4`, includes mql-zmq, `libzmq.dll`, `libsodium.dll`, imports DLL et AutoTrading, puis `READY` et heartbeats du compte attendu. Le terminal est lancé avec un INI temporaire. Pywinauto sert uniquement à l’installation/configuration du pont, pas à la connexion du compte. Un processus terminal seul ne suffit pas à déclarer MT4 connecté.

L’installateur résout le dossier de données, utilise l’archive mql-zmq embarquée, copie, compile dans MetaEditor et peut configurer l’EA. Les contrôles distinguent source, binaire, includes et DLL.

### Faits broker, ticks et symboles MT4

Le bridge fournit compte, terminal, symboles, sélection, spécification broker, tick horodaté, bougies, positions, pending et historique. Le tick vivant et les inventaires live sont la source de vérité. Aucun fichier de reprise ne les remplace. Une donnée indispensable absente entraîne un refus ou report explicite.

### Exécution et Money Management MT4

MT4 couvre MARKET, LIMIT, STOP, entrées uniques/multiples/fractionnées, modifications SL/TP, suppression pending, clôtures totales/partielles et protections MM. MARKET et LIMIT issus d’un signal fractionné restent un seul contexte/lifecycle. Une branche refusée après une branche exécutée produit une exécution partielle.

`MoneyManagementVolumePlanner` reste propriétaire du calcul de volume. Le moteur adapte seulement les contraintes broker.

### Lifecycle MT4 séparé

- `MT4TradeMonitoringMixin` observe les inventaires ;
- `MT4TradeClassifierMixin` classe TP, SL, BE, Smart Close, manuel et inconnu ;
- `MT4TradeLifecycleMixin` possède batchs, TP, pending, transitions, fin et automatismes ;
- `MT4StateReconciler` réconcilie les snapshots sans faire du cache la vérité ;
- les stores MT4 dédiés isolent l’état MT5.

Les événements sont dédupliqués. Une clôture partielle n’est pas recomptée comme terminale. La disparition des MARKET peut annuler les LIMIT sœurs selon le MM, avec notification.

### Dashboard, supervision et notifications

Le Dashboard reste passif et consomme la plateforme active. Les collecteurs communs acceptent MT4/MT5 sans ordre. La supervision reste en lecture seule. Les notifications adaptées à la plateforme passent par l’outbox durable.

### Copy Trader : matrice autorisée

| Source | Cible | État |
|---|---|---|
| MT5 | MT5 | historique conservé |
| MT5 | MT4 | historique conservé |
| MT4 principal | MT5 | source partagée ajoutée |
| MT4 | MT4 | interdit |

`copy_trader_mt4_source_session()` et `MT4SharedSourceObserver` lisent MT4 live. Les mappings protègent contre les doublons sans être une preuve live. Lors d’une clôture partielle, `MT4SourceSnapshotTracker` reconnaît le ticket résiduel, transfère le mapping et réduit la cible MT5 sans réouverture. Le parcours source MT5 reste inchangé.

### Cache et limites

Les caches servent à la performance, reprise ou anti-doublon ; jamais de vérité pour tick, opérations, connexion, cause finale ou mapping actuel. MT4 dépend de l’EA, ZeroMQ, DLL et permissions. Les tests utilisent des fakes. L’absence de bridge/tick frais bloque les décisions concernées. Les 26 échecs d’assertion restent visibles.
...
Les 80 fichiers de langue correspondent à 8 domaines dans 10 langues. La suite comprend 75 modules `test_*.py`.

## 2. Finalité générale et architecture fonctionnelle

NovaBOT est une application de bureau Windows en PyQt5 qui relie Telegram à MetaTrader 5 ou MetaTrader 4, exécute et suit le cycle de vie des opérations sur la plateforme sélectionnée, et propose un Copy Trader selon une matrice de parcours explicitement contrôlée.
...
## 7. Connexion et sélection MetaTrader
...
## 17. Copy Trader multi-plateforme
...
Dans le filtre, plusieurs tests attendent encore `require_numeric_value=False` et un ordre de raisons différent, alors que le code courant place ce critère à `True`. Dans About, le test attend `v2026.0.180` alors que le code affiche `v2026.0.300`. Ces divergences sont des contrats de test non alignés avec le code actuel ; aucune correction n’a été effectuée.
...
Ce rapport décrit le fonctionnement attesté par l’arborescence au 13 août 2026 et n’attribue à NovaBOT aucun comportement absent du code.
